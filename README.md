...
- 붙여넣은 `ridi-at`은 **AES-256-GCM으로 암호화**해 서버(`data/sessions/`)에만 저장하고, 리디 API 호출에만 씁니다. 클라이언트로 다시 내려보내지 않고 로그도 남기지 않습니다.
- 암호화 키는 `RD_SECRET` 환경변수 또는 최초 실행 시 생성되는 `data/.key`. `data/`, `.env`는 git에서 제외됩니다.
- **본인만 쓰는 기기에서 로컬 실행**을 강력히 권장합니다. 공개 배포(특히 Vercel)는 저장소·타임아웃·접근제어 문제가 있어 비권장입니다.
- 쿠키 추출 방법(개발자도구 권장 / 확장프로그램), 유출 시 **쿠키 초기화·무효화 방법**, Vercel 특성, 이 토큰으로 가능한 일과 사용자 책임은 **[SECURITY.md](./SECURITY.md)** 에 자세히 정리했습니다. 사용 전 꼭 읽어주세요.
